...
public class ConferenceEndpoint
{
   ...
   {
      TypedQuery<Conference> findByIdQuery = em.createQuery("SELECT c FROM Conference c LEFT JOIN FETCH c.sessionSubmissions LEFT JOIN FETCH c.vendorPointOfContact LEFT JOIN FETCH c.staff WHERE c.id = :entityId", Conference.class);
   ...
   {
      final List<Conference> results = em.createQuery("SELECT c FROM Conference c LEFT JOIN FETCH c.sessionSubmissions LEFT JOIN FETCH c.vendorPointOfContact LEFT JOIN FETCH c.staff", Conference.class).getResultList();
      return results;
   }
